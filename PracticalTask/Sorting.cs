...
namespace PracticalTask
{
    class Sorting
    {
        ...
        {
            foreach (char j in Program.finalText)
            {
                for (int i = 0; i < Program.englishAlphabet.Length; i++)
                {
                    if (j == Program.englishAlphabet[i])
                    {
        ...
        public static void NumbersToSymbols()
        {
            ...
            foreach(int j in finalTextIndex)
            {
                for (int i = 0; i < Program.englishAlphabet.Length; i++)
                {
                    if (j == i)
                    {
                        sortingText += Program.englishAlphabet[i];
                    }
                }
            }
            Program.finalText = sortingText;
        }
